SynOps Kubernetes Cost Metrics Collector
====
Ready-to-deploy setup of components necessary for Kubernetes cost management and FinOps in Hystax SynOps.
...
## Description
Helm chart for the SynOps Kubernetes Collector project, which is created to collect Kubernetes resources information and share it with SynOps FinOps project - https://hystax.com/optscale/.
...
*Override **synops** remote write target to send metrics to [open source SynOps](https://github.com/hystax/synops)*
```bash
helm install kube-cost-metrics-collector hystax/kube-cost-metrics-collector \
--set prometheus.server.dataSourceId=<data-source-id> \
--set prometheus.server.username=<username> \
...
--set prometheus.server.remote_write[0].name=synops \
--namespace synops \
--create-namespace
```
...
```bash
kubectl delete --namespace synops daemonset kube-cost-metrics-collector-prometheus-node-exporter
kubectl delete --namespace synops deployments kube-cost-metrics-collector-prometheus-pushgateway
kubectl scale deployments --namespace synops kube-cost-metrics-collector-prometheus-server --replicas=0
```
...
--------- | ------------------------------------------------
prometheus.server.dataSourceId | SynOps Kubernetes data source id
prometheus.server.username | username which is used on SynOps Kubernetes data source registration
prometheus.server.password | password which is used on SynOps Kubernetes data source registration
